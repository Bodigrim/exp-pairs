module ExpPairs.Optimize (optimizeWithConstraints, LinearForm (..), RationalForm (..), IneqType (..), Constraint (..), infin, InitPair(..), Path) where
...
evalFunctional :: [RationalForm Rational] -> [Constraint Rational] -> Path -> (Rational, InitPair)
evalFunctional rfs cons path = if rs==[] then (infin, Usual) else minimumBy (comparing fst) rs where
...
optimizeWithConstraints rfs cons = (d, r, ip, path) where
	(r, ip, path) = optimizeWithConstraints' rfs cons (infin, Usual, mempty)
	d = fromRational r
